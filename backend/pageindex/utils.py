...
if not os.getenv("OPENAI_API_KEY") and os.getenv("SARVAM_API_KEY"):
...
def llm_completion(model, prompt, chat_history=None, return_finish_reason=False):
    ...
    if api_base and model and model.startswith("ollama/"):
        kwargs["api_base"] = api_base
    if api_key and model and model.startswith("ollama/"):
        kwargs["api_key"] = api_key

    nvidia_key = os.getenv("NVIDIA_API_KEY") or os.getenv("OLLAMA_API_KEY") or os.getenv("NVIDIA_NIM_API_KEY")
    if nvidia_key and model and ("nvidia" in model or "deepseek" in model):
...
def get_page_tokens(pdf_path, model=None, pdf_parser="PyPDF2", chunks_path=None):
    # ── chunk-driven path (replaces PDF parsing) ──────────────────────────
    if chunks_path is not None:
        ...
        # group texts by page, preserving order
        pages: OrderedDict[int, list] = OrderedDict()
        for chunk in chunks:
        ...
        if not pages:
            raise ValueError("No text content found in chunks")

        min_page, max_page = min(pages), max(pages)
        page_list = []
        for page_num in range(min_page, max_page + 1):
            text         = "\n\n".join(pages.get(page_num, [""]))
            token_length = litellm.token_counter(model=model, text=text)
            page_list.append((text, token_length))

        print(
            f"[PAGEINDEX] Built page_list from chunks: {len(page_list)} pages, "
            f"{sum(t for _, t in page_list):,} total tokens"
        )
        return page_list

    # ── original PDF parsing (unchanged) ─────────────────────────────────
...
async def generate_summaries_for_structure(structure, model=None):
    ...
    async def sem_node_summary(node):
        async with sem:
            return await generate_node_summary(node, model=model)
    tasks = [sem_node_summary(node) for node in nodes]
