...
class PageIndexClient:
    ...
    def route_query(self, query: str, tree: dict, max_pages: int = 30, llm_invoke=None) -> list[int]:
        """
        Agentic routing: given a natural language query and a PageIndex tree structure,
        uses an llm to identify the relevant page numbers in a single call.
        """
        import json
        import re
        from .utils import remove_fields

        def _serialize_tree(nodes, depth=0, max_depth=3):
            if not nodes or depth > max_depth:
                return ""
            lines = []
            for node in nodes:
                indent = "  " * depth
                sp, ep = node.get("start_page", 1), node.get("end_page", 1)
                lines.append(f"{indent}- [{node.get('node_id', '')}] {node.get('title', '')} (pp. {sp}-{ep})")
                summary = (node.get("summary") or "").strip()
                if summary:
                    lines.append(f"{indent}  ↳ {summary[:160]}")
                child_lines = _serialize_tree(node.get("nodes"), depth + 1, max_depth)
                if child_lines:
        ...
        # Detect comprehensive intent
        is_comprehensive = bool(re.search(r'\b(all chapters|detailed|comprehensive|complete report|everything|every detail|full report)\b', query.lower()))
        effective_max_pages = 1000 if is_comprehensive else max_pages

        serialized = _serialize_tree(top_nodes, max_depth=3)
        if not serialized:
            return []

        if is_comprehensive:
            selection_rule = "- The user requested a comprehensive report. Select ALL pages from the outline that contain technical content. Do not filter out any chapters."
        else:
            selection_rule = f"- Select ONLY sections relevant to the user query.\n- Select between 1 and {effective_max_pages} page numbers."
        ...
        try:
            ...
            if raw:
                ...
                all_pages.update({int(p) for p in pages if isinstance(p, (int, float))})
        except Exception as e:
            print(f"[PageIndexClient] Routing failed: {e}")
            
        return sorted(all_pages)[:effective_max_pages]
