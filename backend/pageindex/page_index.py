...
async def check_title_appearance_in_start_concurrent(structure, page_list, model=None, logger=None):
    ...
    async def sem_check_title(title, page_text):
        async with sem:
    ...
    for item in structure:
        if item.get('physical_index') is not None:
            ...
            tasks.append(sem_check_title(item['title'], page_text))
...
def page_list_to_group_text(page_contents, token_lengths, max_tokens=50000, overlap_page=1):    
...
def generate_toc_continue(toc_content, part, model=None):
    ...
    # Truncate the previous tree structure to the last 20 nodes to prevent massive prompt expansion and context overflow
    truncated_toc = toc_content[-20:] if len(toc_content) > 20 else toc_content
    prompt = prompt + '\nGiven text\n:' + part + '\nPrevious tree structure\n:' + json.dumps(truncated_toc, indent=2)
    response, finish_reason = llm_completion(model=model, prompt=prompt, return_finish_reason=True)
...
def process_no_toc(page_list, start_index=1, model=None, logger=None, opt=None):
    ...
    max_tokens = opt.max_token_num_each_node if opt else 100000
...
def process_toc_no_page_numbers(toc_content, toc_page_list, page_list,  start_index=1, model=None, logger=None, opt=None):
    page_contents=[]
    token_lengths=[]
...
async def fix_incorrect_toc(toc_with_page_number, page_list, incorrect_results, start_index=1, model=None, logger=None):
    ...
    sem = asyncio.Semaphore(5)
    async def sem_process_item(item):
        async with sem:
            return await process_and_check_item(item)

    tasks = [
        sem_process_item(item)
...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    # Run checks concurrently with semaphore to prevent local Ollama timeouts
    sem = asyncio.Semaphore(5)
    async def sem_verify_item(item):
        async with sem:
            return await check_title_appearance(item, page_list, start_index, model)

    tasks = [
        sem_verify_item(item)
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if mode == 'process_toc_with_page_numbers':
        ...
        toc_with_page_number = process_toc_no_page_numbers(toc_content, toc_page_list, page_list, model=opt.model, logger=logger, opt=opt)
    else:
    ...
    if accuracy == 1.0 and len(incorrect_results) == 0:
        return toc_with_page_number
    if accuracy > 0.4 and len(incorrect_results) > 0:
...
def page_index_main(doc, opt=None, chunks_path=None):
    ...
    print('Parsing PDF...' if chunks_path is None else 'Loading chunks...')
    page_list = get_page_tokens(doc, model=opt.model, chunks_path=chunks_path)
...
def page_index(doc, model=None, toc_check_page_num=None, max_page_num_each_node=None,
               ...
               if_add_doc_description=None, if_add_node_text=None, chunks_path=None):

    user_opt = {
        arg: value for arg, value in locals().items()
        if arg not in ("doc", "chunks_path") and value is not None
    }
    opt = ConfigLoader().load(user_opt)
    return page_index_main(doc, opt, chunks_path=chunks_path)
